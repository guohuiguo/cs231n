...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        # 1. 创建位置 (i) 张量, 形状 (max_len, 1)
        position = torch.arange(max_len).unsqueeze(1)
        
        # 2. 创建 10000^(-j/d) 部分, 即 div_term
        # torch.arange(0, embed_dim, 2) -> [0, 2, 4, ..., embed_dim - 2]
        div_term = 10000.0 ** (-torch.arange(0, embed_dim, 2) / embed_dim)
        
        # 3. 计算 sin/cos 的参数 (i * div_term)
        # (max_len, 1) * (embed_dim / 2) -> (max_len, embed_dim / 2)
        arg = position * div_term
        
        # 4. 填充 pe 矩阵
        # 偶数索引 (0, 2, 4, ...) 使用 sin
        pe[0, :, 0::2] = torch.sin(arg)
        # 奇数索引 (1, 3, 5, ...) 使用 cos
        pe[0, :, 1::2] = torch.cos(arg)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def forward(self, x):
        ...
        ############################################################################
        # 1. x 形状 (N, S, D), self.pe 形状 (1, max_len, D)
        # 2. 我们需要 self.pe 的前 S 个词元, 即 self.pe[:, :S, :], 形状 (1, S, D)
        # 3. PyTorch 会自动广播 (1, S, D) 来匹配 (N, S, D)
        output = x + self.pe[:, :S, :]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ############################################################################
        q_vectors=self.query(query)
        k_vectors=self.key(key)
        v_vectors=self.value(value)
        q_reshaped=q_vectors.view(N,S,self.n_head,self.head_dim).transpose(1,2)
        k_reshaped=k_vectors.view(N,T,self.n_head,self.head_dim).transpose(1,2)
        v_reshaped=v_vectors.view(N,T,self.n_head,self.head_dim).transpose(1,2)
        # 计算q与k的点积
        ...
        # 权重与value
        context = torch.matmul(attn_weights, v_reshaped)
        # 变形状
        context = context.transpose(1, 2).contiguous()
        
        context_reshaped = context.view(N, S, self.emd_dim)
        # 最后映射回输出的词汇空间
        output = self.proj(context_reshaped)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        ############################################################################
        # 1. 交叉注意力块 (Cross-Attention Block)
        # Query 来自解码器 (tgt), Key 和 Value 来自编码器 (memory)
        shortcut = tgt
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # 1. 将 H 和 W 维度拆分为 (网格维度, patch 维度)
        # P = self.patch_size
        # num_patches_h = H // P
        # num_patches_w = W // P
        # (N, C, H, W) -> (N, C, num_patches_h, P, num_patches_w, P)
        x_reshaped = x.reshape(N, C, H // self.patch_size, self.patch_size, W // self.patch_size, self.patch_size)
        
        # 2. 调整维度顺序，将所有 patch 维度 (C, P, P) 组合在一起
        # (N, C, num_patches_h, P, num_patches_w, P) -> (N, num_patches_h, num_patches_w, C, P, P)
        x_permuted = x_reshaped.permute(0, 2, 4, 1, 3, 5)
        
        # 3. 展平 (flatten) patch 网格维度和 patch 内容维度
        # (N, num_patches_h, num_patches_w, C, P, P) -> (N, num_patches, patch_dim)
        # self.num_patches = num_patches_h * num_patches_w
        # self.patch_dim = C * P * P
        patches_flattened = x_permuted.reshape(N, self.num_patches, self.patch_dim)
        
        # 4. 使用线性层进行投影
        # (N, num_patches, patch_dim) -> (N, num_patches, embed_dim)
        out = self.proj(patches_flattened)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return out
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        ############################################################################
        # 1. 自注意力块 (Self-Attention Block)
        # (Query, Key, Value) 都来自 src
        shortcut = src
