...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        # 应用公式: x_0 = (x_t - sqrt(1 - alpha_bar_t) * noise) / sqrt(alpha_bar_t)
        # 注意: (self.sqrt_alphas_cumprod 在 __init__ 中有定义)
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        # 应用公式: noise = (x_t - sqrt(alpha_bar_t) * x_start) / sqrt(1 - alpha_bar_t)
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        ##################################################################
        # 1. 获得 U-Net 模型的预测输出
        model_output = self.model(x=x_t, time=t, model_kwargs=model_kwargs)

        # 2. & 3. 无论模型目标是什么，都计算出 pred_x_start 并进行 clamp
        if self.objective == "pred_noise":
            # 模型预测的是噪声
            pred_noise = model_output
            # 从 x_t 和预测的噪声中，计算出 x_0
        ...
        # 步骤 3: Clamp 预测的 x_start 到 [-1, 1] 范围
        pred_x_start = pred_x_start.clamp(-1.0, 1.0)

        # 步骤 4: 使用 q_posterior 计算后验分布的均值和标准差
        # q_posterior 内部会使用 __init__ 中预先计算的系数
        posterior_mean, posterior_std = self.q_posterior(
            x_start=pred_x_start, x_t=x_t, t=t
        )

        # 4. (续): 使用重参数技巧 (reparameterization trick) 进行采样
        # x_{t-1} = mean + std * noise
        # 注意: 当 t=0 时, __init__中计算的 posterior_std[0] 应该为 0
        # 所以 noise * 0 = 0，x_{t-1} 就等于 posterior_mean
    ...
    def q_sample(self, x_start, t, noise):
        ...
        ####################################################################
        # 假设 'extract' 函数在
        # from cs231n.utils import extract
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        ####################################################################
        # 1. 使用 q_sample 从 x_0 和 noise 生成带噪图像 x_t
        x_t = self.q_sample(x_start=x_start, t=t, noise=noise)

        # 2. 将 x_t 和 t (以及条件) 喂给 U-Net 模型，得到预测值
        model_output = self.model(x=x_t, time=t, model_kwargs=model_kwargs)

        # 3. 计算加权均方误差 (Weighted MSE)
        #    target 是模型应该预测的目标 (在 setup 中已定义为 noise 或 x_start)
        #    model_output 是模型的实际预测值
        #    loss_weight 是预先计算的、基于 t 的权重
        loss = (loss_weight * (target - model_output) ** 2).mean()
        ####################################################################
