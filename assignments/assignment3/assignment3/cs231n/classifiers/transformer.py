...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # (N, W) -> (N, 1, W)
        # 将其 unsqeeze 以便 TransformerDecoder 能将其作为 "memory" (序列长度为1)
        memory = projected_features.unsqueeze(1)
        
        # 2. 准备掩码 (tgt_mask)
        # torch.tril 创建一个下三角矩阵。值为 1 的地方是允许关注的。
        # MultiHeadAttention 会将 mask == 0 的地方填充为 -inf
        # (T, T)
        tgt_mask = torch.tril(torch.ones(T, T, device=captions.device))
        
        # 3. 应用解码器并投影到 scores
        # tgt=(N, T, W), memory=(N, 1, W), tgt_mask=(T, T) -> (N, T, W)
        decoder_output = self.transformer(caption_embed_pos, memory, tgt_mask=tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # 1. 将图像转换为 patch 向量
        # (N, C, H, W) -> (N, S, D)  (S=num_patches, D=embed_dim)
        patches = self.patch_embed(x)
        
        # 2. 添加位置编码
        # (N, S, D) -> (N, S, D)
        patches_pos = self.positional_encoding(patches)
        
        # 3. 通过 Transformer encoder
        # (N, S, D) -> (N, S, D)
        encoder_output = self.transformer(patches_pos, src_mask=None)
        
        # 4. 平均池化 (按提示要求)
        # (N, S, D) -> (N, D)
        # 沿着序列 (S) 维度 (dim=1) 进行平均
